...
void fun2(vector<string> v) {
	...
	if (prince == 1) {//ID
		...
		return;
		 ...
		 tree.insert_year( into);
	}
	else if (prince == 3) {// deaths
		tree.insert_death(into);
	}
	else if (prince == 4) {// age-dactor

	tree.insert_age(into);
	}
	else if (prince == 5) {// state
		tree.insert_state( into);
	}
	else if (prince == 6) {//cause name
		tree.insert_cause_name( into);
	}
	else if (prince == 7) {
		tree.insert_cause_113( into);
...
t:
	...
	death:
		...
		if (search == 1) {
			...
			cout << "Enter key to Delete" << endl;
			cin >> know;
			tree.delete_deaths(tree.root, know);
			goto death;
		}
		/*else if (search == 5) {
			tree.levelorder_newline();
